...
import { ServicePage, NAJEM } from "../../../components/wzory/pages/service";
...
export async function generateMetadata({ params }: { params: Promise<{ wzor: string }> }): Promise<Metadata> {
  ...
  return { title: `Obsługa najmu | ${w?.office ?? "Wzór"} | wzór AgentSpace`, robots: { index: false, follow: true } };
}
...
export default async function Page({ params }: { params: Promise<{ wzor: string }> }) {
  ...
  return <ServicePage wzor={w.slug} c={NAJEM()} />;
}
