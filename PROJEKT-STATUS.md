...
- **Umowa rezerwacyjna** (`/app/rezerwacje`) — generator umowy rezerwacyjnej **sprzedaż/najem** (wybór), typ nieruchomości, powtarzalne strony (właściciele/kupujący). Prawnie kompletna: opłata rezerwacyjna NIE jest zadatkiem (art. 394 KC), zasady przepadku/zwrotu, akt notarialny (art. 158 KC) dla sprzedaży, najem okazjonalny (art. 19a). Definicje stron w formie standardowej (Sprzedający/Kupujący, Wynajmujący/Najemca) — bez błędów gramatycznych. `lib/reservation.ts` = builder treści; druk/PDF przez `print-sheet` (window.print). Bez DB, client-side.
